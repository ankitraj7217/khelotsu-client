...
import { addUsersInRoom, isPersonAllowedInRoom, removeUserInRoom } from "../../../Network/roomApiCalls";
...
const PeopleDetails: FC<IRoomUsers> = ({ names, setErrorMsg }) => {
    ...
    const _handleUserNameInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const _onAddUser = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (usersInRoom.includes(userNameAddInput)) throw new Error("User already available in room.")
    ...
    const _onRemoveUser = async(username: string) => {
        try {
            ...
            setUsersInRoom(prevUsers => prevUsers.filter(prevUser => prevUser !== username));
        } catch (error: any) {
            setErrorMsg(error?.message);
        }
    }
    ...
    return (
        <section className="users-details">
                    ...
                    usersInRoom.map((username: string) => {
                        return (
                            <div className="users-details-display-user" key={username}>
                                ...
                                <div className="users-details-display-user__remove" 
            ...
            <div className="users-details-add-user">
                <form action="" className="users-details-add-user-form" onSubmit={_onAddUser}>
                    <div className="users-details-add-user-form-input">
                        <CustomInput label={t("ENTER_USERNAME")} inputVal={userNameAddInput}
                            onInputChange={_handleUserNameInputChange} />
                    </div>
                    <button className="users-details-add-user-form-submit">Add</button>
                </form>
