...
export class WebRTCConnection {
  ...
  async receiveIceCandidate(
    ...
    try {
      const connectionID = `${offererUserName}-${answererUserName}`;
      const peerConnectionObj = this.peerConnection.find(
        (ele) => ele.connectionID === connectionID
      );
      const peerConnection = peerConnectionObj?.peerConnection;
      console.log(
        "this.peerConnection in receiveIce Candidate: ",
        connectionID,
        this.peerConnection
      );
  ...
  removeUserConnection(userName: string) {
    try {
      this.peerConnection = this.peerConnection.filter(
        ({ connectionID }) => !connectionID.includes(userName)
      );
