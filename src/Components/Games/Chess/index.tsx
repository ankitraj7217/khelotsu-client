...
import { getInitialChessSymbol, receiveChessPos, requestInitialChessSymbol, sendChessPos, socketGenericCheck } from "../../../Utils/socketUtils";
import { useDispatch, useSelector } from "react-redux";
import { clearBoard, setPlayerSymbols, updateChessBoard, updateChessBoardEntirely } from "../../../ReduxStore/Slices/chessGameSlice";
...
const Chess: FC<IGame> = ({ socket, setErrorMsg, personsAllowedInRoom }) => {
    ...
    const { chessBoard, playerSymbols } = useSelector((state: RootState) => state.chessGame);
    ...
    const _onDrop = (e: any) => {
        ...
        try {
            if (!socket) throw new Error("Socket not yet avaiable.");
            sendChessPos(socket, JSON.stringify({
                piece: incomingPiece,
        ...
        <section className="chess">
            <div className="chess-game">
                <div className="chess-game-ranks">
                    {
                        chessRanks.map(ele => <div key={9 - ele} className="chess-game-ranks-rank">{9 - ele}</div>)
                    }
                </div>
                <div className="chess-game-board" onDrop={_onDrop} onDragOver={_onDragOver}
                    onMouseOver={_onMouseOver} onMouseLeave={_onMouseLeave}>
                    {
                        chessRanks.flatMap((rank, idx1) =>
                            chessFiles.map((file, idx2) => {
                                const bgColorClassName = (idx1 + idx2) % 2 === 0 ? "chess-game-col-l" : "chess-game-col-d"
                ...
                <div className="chess-game-files">
                    {
                        chessFiles.map(ele => <div key={ele} className="chess-game-files-file">{ele}</div>)
                    }
                </div>
            </div>
            <div className="chess-start">
                <button className="chess-start-btn" onClick={_startGame}>Start</button>
            </div>
            {
